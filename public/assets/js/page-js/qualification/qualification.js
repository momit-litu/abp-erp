...
$(document).ready(function () {
	...
	$("#save_qualification").on('click',function(){
		...
		if($.trim($('#code').val()) == ""){
			success_or_error_msg('#master_message_div','danger',"Please Insert Qualification code","#code");
		}
		else if($.trim($('#title').val()) == ""){
			success_or_error_msg('#master_message_div','danger',"Please enter qualification name","#title");
		}
		else if($.trim($('#tqt').val()) == "" || !($.isNumeric($('#tqt').val()))){
			success_or_error_msg('#master_message_div','danger',"Please enter TQT","#tqt");
		}
		else if($.trim($('#total_credit_hour').val()) == "" || !($.isNumeric($('#total_credit_hour').val()))){
			success_or_error_msg('#master_message_div','danger',"Please enter total credit hour","#total_credit_hour");
		}
		else if(parseFloat($('#glh').val()) > parseFloat($('#tqt').val())){
			success_or_error_msg('#master_message_div','danger',"TQT cannot less than GLH","#tqt");
		}
		else if($.trim($('#registration_fees').val()) == "" || !($.isNumeric($('#registration_fees').val()))){
			success_or_error_msg('#master_message_div','danger',"Please enter registration fees","#registration_fees");
		}
		else{
			$.ajax({
	...
	$("#admin_user_list_button, #cancel_button").on('click',function(){
		...
		$('#unit_table tr:gt(0)').remove();
		$("#save_qualification").html('Save');
		$("#admin_user_add_button").html("<b> Add Qualification</b>");
	});
	 ...
	 qualificationView = function qualificationView(id){	
		$.ajax({
			...
			success: function(response){
				...
				var modalHtml  ="<div class='col-lg-12 margin-top-5'><div class='col-lg-3 col-md-4 '><strong>Qualification Code :</strong></div>"+"<div class='col-lg-9 col-md-8'>"+data['code']+"</div></div>";
					modalHtml +="<div class='col-lg-12 margin-top-5'><div class='col-lg-3 col-md-4 '><strong>Qualification Title :</strong></div>"+"<div class='col-lg-9 col-md-8'>"+data['title']+"</div></div>";
					modalHtml +="<div class='col-lg-12 margin-top-5'><div class='col-lg-3 col-md-4 '><strong>Level :</strong></div>"+"<div class='col-lg-9 col-md-8'>"+data['level']['name']+"</div></div>";
					modalHtml +="<div class='col-lg-12 margin-top-5'><div class='col-lg-3 col-md-4 '><strong>TQT :</strong></div>"+"<div class='col-lg-9 col-md-8'>"+data['tqt']+"</div></div>";
					modalHtml +="<div class='col-lg-12 margin-top-5'><div class='col-lg-3 col-md-4 '><strong>Total Credit Hour :</strong></div>"+"<div class='col-lg-9 col-md-8'>"+data['total_credit_hour']+"</div></div>";
