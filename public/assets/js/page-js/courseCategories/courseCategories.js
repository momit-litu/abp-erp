...
$(document).ready(function () {
    ...
    course_categories_datatable = $('#courseCategories_table').DataTable({
        destroy: true,
        "order": [[ 0, 'desc' ]],
        "processing": true,
        "serverSide": false,
        "ajax": url+"/course-categories",
        "aoColumns": [
            { mData: 'id'},
            { mData: 'title'},
            { mData: 'description'},
    ...
    $("#save_courseCategory").on('click',function(){
        ...
        var formData = new FormData($('#courseCategory_form')[0]);


        if($.trim($('#title').val()) == ""){
            success_or_error_msg('#form_submit_error','danger',"Please enter Title","#first_name");
        }
        else{
            $.ajax({

                url: url+"/course-category",
                type:'POST',
                data:formData,
                async:false,
                cache:false,
    ...
    courseCategoryAdd = function studentAdd(){
        $("#form-title").html('<i class="fa fa-plus"></i> Add  New Course Category');
        $("#clear_button").show();
