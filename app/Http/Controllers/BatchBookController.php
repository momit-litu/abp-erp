...
class BatchBookController extends Controller
{
    ...
    public function studenBookList($batchId)
    {
        ...
        GROUP_CONCAT(student_book_details) student_book_details
            FROM (
                SELECT  s.name AS student_name, bs.student_enrollment_id, bs.status AS student_status,
                ...
                foreach($bookInfoArr as $bookInfo){
                    ...
                    if(count($bookfeedbacks)>0){
                        foreach($bookfeedbacks as $bookfeedback){
                            $feedbackHtml .=$bookfeedback->feedback."</br>";
                        }
                    }

                   $addFeedbackButton =  "<div class='col-md-12 text-right'><a href='javascript:void(0)' title='Edit' onclick='addFeedback(".$studentBookId.")'  class='btn btn-xs btn-hover-shine  btn-default' >+</a></div>";
