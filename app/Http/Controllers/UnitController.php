	   ...
	   $units = Unit::Select('id','unit_code', 'name', 'glh', 'tut', 'assessment_type', 'status')
        ...
        foreach($units as $unit){
            ...
            if($edit_permisiion>0){
                $data['actions'] .="<button onclick='unitEdit(".$unit->id.")' id=edit_" . $unit->id . "  class='btn btn-xs btn-green module-edit' ><i class='clip-pencil-3'></i></button>";
            }
            if ($delete_permisiion>0) {
                $data['actions'] .=" <button onclick='unitDelete(".$unit->id.")' id='delete_" . $unit->id . "' class='btn btn-xs btn-danger' ><i class='clip-remove'></i></button>";
            }
